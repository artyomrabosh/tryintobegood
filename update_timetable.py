...
# тут всё делается через апи сайта lightpdf и файлообменник alibaba
...
def create_task(uploaded):
    ...
    query_files = urllib.parse.quote(json.dumps(files))
    params = {
        'service_type': "pdf-to-word",
        'files': query_files,
        'autostart': 1,
        'args': ''
    }
...
def download_excel(task):
    ...
    return requests.get(task_link,headers=cfg.auth_header_lightpdf)
